...
//                    openLocation(longitude, latitude);
    ...
    private void openLocation(double longitude, double latitude) {
        if (latitude > 0) {
            if (locationTrack.canGetLocation()) {
                getCompleteAddressString(latitude, longitude);
                String url = "https://www.google.com/maps/dir/?api=1&destination=" + longitude + "," + latitude + "&travelmode=driving";
                        ...
                        public void onSuccess(UploadTask.TaskSnapshot taskSnapshot) {
                            Task<Uri> downloadUrl = taskSnapshot.getStorage().getDownloadUrl().addOnCompleteListener(new OnCompleteListener<Uri>() {
                                ...
                                public void onComplete(@NonNull Task<Uri> task) {
                                    ...
                                    uploadinfo imageUploadInfo = new uploadinfo(TempImageName, task.getResult().toString(),ComLocation,timeUpload,longitude,latitude);
